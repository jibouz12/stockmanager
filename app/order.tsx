  ...
  return (
    <View style={styles.hiddenItem}>
      ...
      >
        ...
        <Text style={styles.restoreButtonText}>{t('order.restore')}</Text>
      </TouchableOpacity>
    </View>
  );
}
...
export default function OrderScreen() {
  ...
  const handleUpdateQuantity = async (itemId: string, newQuantity: number) => {
    try {
      ...
      Alert.alert(t('error.title'), t('error.addProduct'));
    }
  };
        ...
        { 
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(t('error.title'), t('error.removeProduct'));
            }
          }
        }
      ]
  ...
  const handleRestoreItem = async (productId: string) => {
    try {
      ...
      Alert.alert(t('success.title'), 'Suggestion restaurée dans la commande');
    } catch (error) {
      console.error('Erreur lors de la restauration:', error);
      Alert.alert(t('error.title'), 'Impossible de restaurer la suggestion');
    }
  };
  ...
  const renderHeader = () => (
    <View style={styles.header}>
        ...
        {hiddenItems.length > 0 && (
          ...
          >
            ...
            <Text style={styles.statText}>
              {hiddenItems.length} {hiddenItems.length > 1 ? t('order.hiddens') : t('order.hidden')}
            </Text>
          </TouchableOpacity>
        )}
      </View>
      ...
      <View style={styles.actionButtonsContainer}>
        ...
        >
          <Text style={styles.actionButtonPrimaryText}>{t('order.placeOrder')}</Text>
        </TouchableOpacity>
      </View>
      ...
      >
        <View style={styles.addProductBannerContent}>
          ...
          <View style={styles.addProductBannerText}>
            <Text style={styles.addProductBannerTitle}>{t('order.addProduct')}</Text>
            <Text style={styles.addProductBannerSubtitle}>
              {t('order.addProductDesc')}
            </Text>
          </View>
        </View>
      </TouchableOpacity>
    ...
    <SafeAreaView style={styles.container}>
      <FlatList
        data={[
          ...
          ...(autoItems.length > 0 ? [{ type: 'section', title: t('order.autoSuggestions'), count: autoItems.length, description: t('order.autoSuggestionsDesc') }] : []),
          ...autoItems.map(item => ({ type: 'item', data: item })),
          ...(manualItems.length > 0 ? [{ type: 'section', title: t('order.manualItems'), count: manualItems.length, description: t('order.manualItemsDesc') }] : []),
          ...manualItems.map(item => ({ type: 'item', data: item })),
          ...(showHidden && hiddenItems.length > 0 ? [{ type: 'section', title: t('order.hiddenSuggestions'), count: hiddenItems.length, description: t('order.hiddenSuggestionsDesc') }] : []),
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyTitle}>{t('order.noOrder')}</Text>
